...
def index():
    if request.method == 'POST':
        ...
        ax4.plot(df.index, df['Volume'], color='blue')
        ax4.set_title("\U0001F50A Volume Over Time")
        ax4.set_ylabel("Volume")
        volume_path = "static/volume_chart.png"
        fig4.savefig(volume_path)
        plt.close(fig4)
        extra_charts.append({'title': "Volume Over Time", 'path': 'volume_chart.png'})

        # Plot 5: Daily Returns
        df['Daily Return'] = df['Close'].pct_change()
        fig5, ax5 = plt.subplots(figsize=(12, 5))
        df['Daily Return'].plot(ax=ax5, color='purple')
        ax5.set_title("\U0001F4CA Daily Returns")
        returns_path = "static/daily_returns.png"
        fig5.savefig(returns_path)
        plt.close(fig5)
        extra_charts.append({'title': "Daily Returns", 'path': 'daily_returns.png'})
